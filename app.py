...
def priority_score(d):
    hazard_weight = {
        ...
        str(d.get("hazard_level", "LOW")).upper(),
        0.35
    )
    ...
    area = max(
        float(d.get("area_sq_m") or 0),
        0
    )

    size_factor = (
        min(
            math.log1p(area) / math.log(11),
            1.0
        )
        if area
        else 0.0
...
def enrich_detection(d):
    ...
    # This is an explainability heuristic,
    # not a trained shadow classifier.
    x["shadow_evidence"] = round(
        clamp(
            35
            + float(x.get("confidence", 0)) * 0.55
            + min(
                float(x.get("area_sq_m") or 0) * 2,
                10
            ),
            0,
            100
...
def detect():
    ...
    try:
        ...
        if engine is None:
            return jsonify({
                "success": False,
                "error": f"Model not loaded: {engine_error}"
            }), 500
        ...
        print(
            f"[DETECT] file={upload_path}"
        )

        print(
            f"[DETECT] conf={conf_thresh}, "
            f"range={range_meters}, "
            f"lat={base_lat}, "
            f"lon={base_lon}"
        )
        ...
        return jsonify({
            ...
            "result_image":
                f"/static/results/{output_name}",

            # New: information used for the scan.
            # This is also used by JSON/CSV exports.
            "scan_parameters": {
                "sonar_range_m":
                    round(range_meters, 2),
            ...
            "processing": {
                "noise_filter":
                    "Bilateral acoustic noise reduction",

                "detector":
                    "YOLO",

                "geolocation":
                    "Range-based image-to-coordinate projection",

                "explainability":
                    "Heuristic evidence layer",
            },
        })
...
def list_surveys():
    ...
    if q:
        ...
        query = query.filter(
            or_(
                Survey.survey_name.ilike(
                    like
                ),
                Survey.original_filename.ilike(
                    like
                )
            )
        )
...
@app.route(
    "/api/surveys/<int:survey_id>"
)
def get_survey(survey_id):

    survey = db.session.get(
        Survey,
        survey_id
    )
...
@app.route(
    "/api/anomalies/<int:anomaly_id>/review",
    methods=["POST"]
)
def review_anomaly(anomaly_id):

    anomaly = db.session.get(
        Anomaly,
        anomaly_id
    )
...
def recent_detections():
    ...
    rows = (
        ...
        .query(
            Anomaly,
            Survey.created_at,
            Survey.original_filename
        )
        .join(
            Survey,
            Anomaly.survey_id == Survey.id
        )
        .order_by(
            Survey.created_at.desc(),
            Anomaly.id.desc()
...
# ============================================================
# ANOMALY CONTEXT / HISTORY
# ============================================================

@app.route(
    "/api/anomalies/<int:anomaly_id>/context"
)
def anomaly_context(anomaly_id):
    ...
    # Spatial history:
    # same class within a small coordinate window.
    delta = 0.00025
    ...
    nearby = (
        ...
        .filter(
            ...
            Anomaly.latitude.between(
                target.latitude - delta,
                target.latitude + delta
            ),

            Anomaly.longitude.between(
...
def hotspots():
    ...
    # Approximate display grid;
    # not a scientific density estimator.
    for a in rows:
...
def download_json():
    ...
    # --------------------------------------------------------
    # Preserve existing summary values where available
    # --------------------------------------------------------
    ...
    export_summary = {
        ...
        "critical":
            hazard_counts["critical"],

        "high":
            hazard_counts["high"],

        "medium":
            hazard_counts["medium"],
    ...
    return output.getvalue(), 200, {
        ...
        "Content-Disposition":
            (
                "attachment; "
                "filename="
                "varun_netra_anomaly_report.csv"
            ),
    }
